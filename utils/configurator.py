# -*- coding: utf-8 -*-
# Description: configurator for crescent
# Author: @email: enoche.chow@gmail.com
# Update: 20260522
...
class Config(object):
    ...
    def _load_dataset_model_config(self, config_dict, mg):
        '''
            Merge the config files from the dataset and model and overall.yaml config file.
        '''
            ...
            if os.path.isfile(file):
                with open(file, 'r', encoding='utf-8') as f:
                    ...
                    if fdata.get('hyper_parameters'):
                        hyper_parameters.extend(fdata['hyper_parameters']) # hyper_parameters list
                    file_config_dict.update(fdata)
                    
        file_config_dict['hyper_parameters'] = hyper_parameters # avoid override by duplicate hyper_parameters files
        return file_config_dict

    def _build_yaml_loader(self):
        '''
            Fix YAML parsing: ensure scientific notation (e.g. 1e-3) is parsed as float
        '''
